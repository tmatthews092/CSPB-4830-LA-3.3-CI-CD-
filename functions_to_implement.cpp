...
std::vector<std::string> Split(std::string whole, std::string separator)
{
	...
	{
		if (std::string(1, whole[i]) != separator)
		{
			eltstr += std::string(1, whole[i]);
		}
		else if (eltstr.size())
		{
			toReturn.push_back(eltstr);
			eltstr = "";
		}
	}
	if (eltstr.size())
...
// takes a vector of doubles and removes all elements evenly divisible by the passed in double
std::vector<double> MultiplesFilter(std::vector<double> a, double divides_by)
{
	if (divides_by == 0)
	{
		return a;
	...
	for (unsigned int i = 0; i < a.size(); i++)
	{
		double to_compare = a[i] / divides_by;
		if (std::floor(to_compare) == to_compare)
		{
			a.erase(a.begin() + i);
		}
	}
...
std::vector<bool> GreaterMask(std::vector<double> nums, double greater_than)
{
	std::vector<bool> greater_than_vector = std::vector<bool>();
	for (double num : nums)
	{
		greater_than_vector.push_back(num > greater_than);
	}
	return greater_than_vector;
}
